...
@property (nonatomic, strong) UIViewController *requester;
...
-(void)makeHud{
    ...
    [_testActivityIndicator setFrame:CGRectMake(0, 0, 100, 100)];//不建议这样设置，因为UIActivityIndicatorView是不能改变大小只能改变位置，这样设置得到的结果是控件的中心在（100，100）上，而不是和其他控件的frame一样左上角在（100， 100）长为100，宽为100.
     [_imageView addSubview:_testActivityIndicator];
...
{
    [self presentCropImageViewControllerWithAnimation:animation];
}
...
- (void)disappear
{
    [self dismissCropImageViewControllerAnimated:YES];
}
...
{
    if ([self.delegate respondsToSelector:@selector(cropImageController:didCancelCropImageWithRequester:)]) {
        [self.delegate cropImageController:self didCancelCropImageWithRequester:self.requester];
    } else {
        [self disappear];
    }
}
...
//    _pr(cropRect);
    ...
    if ([self.delegate respondsToSelector:@selector(cropImageController:requester:didFinishCropImage:originalImage:)]) {
        [self.delegate cropImageController:self requester:self.requester didFinishCropImage:cropImage originalImage:self.image];
        return;
    }
